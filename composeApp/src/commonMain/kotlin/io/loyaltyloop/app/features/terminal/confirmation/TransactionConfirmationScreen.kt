            ...
            Column(
                ...
                Card(
                    ...
                    Column(modifier = Modifier.padding(16.dp)) {
                        InfoRow(stringResource(Res.string.term_confirm_purchase), "${calculation.purchaseAmount}")
                        
                        if (calculation.pointsSpent > 0) {
                            Spacer(modifier = Modifier.height(8.dp))
                            InfoRow(stringResource(Res.string.term_confirm_spent), "-${calculation.pointsSpent}", MaterialTheme.colorScheme.error)
                        }
                        
                        if (calculation.pointsToAward > 0) {
                            Spacer(modifier = Modifier.height(8.dp))
                            InfoRow(stringResource(Res.string.term_confirm_earned), "+${calculation.pointsToAward}", MaterialTheme.colorScheme.primary)
                        }
                        ...
                        Row(
                            ...
                            Text(
                                text = "${calculation.moneyPaid}", // Можно добавить валюту если прокинуть
                ...
                OutlinedCard(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        InfoRow(stringResource(Res.string.term_confirm_balance), "${calculation.newBalance}")
