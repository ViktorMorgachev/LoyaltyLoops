...
class TransactionService(
    ...
    suspend fun scanQr(cashierUserId: String, request: ScanQrRequest): ScanQrResponse {
        ...
        val partner = partnerRepository.getPartnerById(partnerIdFromRepo)
            ?: throw LoyaltyException(AppErrorCode.BUSINESS_NOT_FOUND)
        ...
        val settings = partnerRepository.getSettingsByPointId(request.tradingPointId)
            ?: throw LoyaltyException(AppErrorCode.LOYALTY_SETTING_NOT_FOUND, "Loyalty settings not found")
    ...
    suspend fun calculateTransaction(
        ...
        return LoyaltyCalculator.calculate(card, settings, purchaseAmount, strategy)
    }
    ...
    suspend fun processTransaction(
        ...
        val calc = LoyaltyCalculator.calculate(card, settings, purchaseAmount, strategy)

        if (calc.message.startsWith("Invalid Amount") || calc.message.startsWith("Error")) {
             throw LoyaltyException(AppErrorCode.INVALID_AMOUNT, calc.message)
        }
        ...
        if (strategy == TransactionStrategy.VISIT) {
            val target = settings.visitsTarget ?: throw LoyaltyException(AppErrorCode.NOT_FOUND, "VisitTarget not set")
    ...
    suspend fun getAnalytics(ownerId: String, period: io.loyaltyloop.shared.models.AnalyticsPeriod): io.loyaltyloop.shared.models.AnalyticsResponse {
        val partner = partnerRepository.getPartnerByUserId(ownerId)
    ...
    private fun parseQrCode(qrContent: String): QrData {
        ...
        return QrData(
            userId = parts[1],
            timestamp = parts[2].toLongOrNull() ?: throw LoyaltyException(AppErrorCode.INVALID_QR_SIGNATURE, "Invalid timestamp"),
            signature = parts[3]
        )
    }
