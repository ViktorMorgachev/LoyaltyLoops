...
## 1. Обзор и Концепция
Система внедряет поддержку работы партнеров в разных странах (KG, KZ, UZ, RU, BY) с разными валютами филиалов.
...
*   **TTL (Time To Live):** **25 часов**.
    *   *Зачем:* Механизм самоочистки. Если партнер перестает работать с валютой, джоб перестает её обновлять, и старый ("зомби") курс автоматически удаляется через сутки.
...
object PartnersTable : Table("partners") {
    ...
    // [NEW] Политика: Начислять ли бонусы, если в чеке было списание баллов?
    val allowEarnOnBurn = bool("allow_earn_on_burn").default(true)
}
```
...
3.  **Burn Logic:**
    ...
    *   Проверка: `Balance (10 USD) >= 5.0 USD`.
4.  **Earn Logic:**
    *   Остаток деньгами: `5000 - 425 = 4575 KGS`.
    *   Кэшбэк 10%: `457.5 KGS`.
