...
*   **[🛠 Руководство по Сборке и Деплою (Deployment)](docs/operations/DEPLOYMENT.md)** — Как запустить проект локально и на сервере.
*   **[🔄 Архитектура и Флоу (Technical Flow)](docs/flows/LOYALTY_FLOW.md)** — Как это работает под капотом (QR, сокеты, транзакции).
*   **[💼 Возможности для Партнеров (Features)](docs/PARTNER_GUIDE.md)** — Описание бизнес-функционала (CRM, Стратегии, Аналитика).
*   **[🤖 Правила ведения проекта (Claude Orchestration)](CLAUDE.md)** — Skill map, рабочий цикл, структура документации.
*   **[🧾 Технический долг](docs/TECH_DEBT.md)** — Реестр TD-N: читать перед каждой фичей.
...
./gradlew :composeApp:clean :composeApp:podInstall
...
```
Собрать/запустить в Xcode:
Открыть iosApp/iosApp.xcworkspace.
Схема iosApp, SDK — Simulator, например Destination: iPhone 15 (iOS 18.0).
Product > Clean Build Folder, затем Run
...
Copyright (c) 2025 LoyaltyLoops. All Rights Reserved.

**ВНИМАНИЕ**: Вся информация, содержащаяся здесь, является и остается собственностью LoyaltyLoops.
...
Распространение этой информации или воспроизведение этого материала **строго запрещено** без предварительного письменного разрешения от LoyaltyLoops.
