...
class AnalyticsService(
    ...
    suspend fun getAnalytics(
        ...
        fun toBase(amount: Double, rate: Double): Double = if (rate > 0) amount / rate else 0.0

        val totalRevenue = transactions.sumOf { toBase(it.amount, it.exchangeRateSnapshot) }
        ...
        val chartData = labels.map { label ->
            ...
            io.loyaltyloop.shared.models.RevenueChartPoint(
                date = label,
                revenue = pointsInLabel.sumOf { toBase(it.amount, it.exchangeRateSnapshot) },
