...
export const PartnerStaffPage = () => {
    ...
    useEffect(() => {
        if (tab === 0) loadManagers();
        if (tab === 1) loadCashiers();
    }, [tab]);
    ...
    const uniqueCashiers = React.useMemo(() => {
        ...
        cashiers.forEach(c => {
            ...
            entry.transactionsCount += (c.transactionsCount || 0);
        });
                                ...
                                managers.map((m) => (
                                    <TableRow key={m.id}>
                                        ...
                                        <TableCell align="right">
                                            <Button color="error" onClick={() => handleFireManager(m.id)}>
                                                {t('staff.fire')}
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell>{t('admin.total_revenue')}</TableCell>
                                <TableCell>{t('admin.total_transactions')}</TableCell>
                                <TableCell>{t('staff.active')}</TableCell>
                                <TableCell align="right">{t('common.actions')}</TableCell>
                            </TableRow>
                        </TableHead>
                            ...
                            {uniqueCashiers.length === 0 ? (
                                ...
                                uniqueCashiers.map((u: any) => (
                                    <TableRow key={u.userId}>
                                        ...
                                        <TableCell>{u.revenue.toFixed(0)}</TableCell>
                                        <TableCell>{u.transactionsCount}</TableCell>
                                        <TableCell>{u.active ? t('common.yes') : t('common.no')}</TableCell>
                                        <TableCell align="right">
                                            <Button color="error" onClick={() => handleFireEmployee(u)}>
                                                {t('staff.fire')}
                                            </Button>
                                        </TableCell>
                                    </TableRow>
