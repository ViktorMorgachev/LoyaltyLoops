...
## TD-003 — Нет блокировок при денежных операциях

- **Область:** server / TransactionService, LoyaltyCardRepository
- **Проблема:** списание баллов делает проверку баланса и update без `SELECT ... FOR UPDATE` и без optimistic locking. Начисление атомарно (SQL-инкремент), списание — нет.
- **Почему это техдолг:** два параллельных списания с одной карты могут обе пройти проверку баланса.
- **Риск:** отрицательный баланс, прямые финансовые потери партнёров.
- **Приоритет:** 🔴 Critical
- **Что нужно сделать:** `forUpdate()` на строку карты внутри транзакции списания + CHECK-констрейнт `balance >= 0` в БД как страховка. Покрыть конкурентным тестом.
- **Когда делать:** до роста нагрузки на транзакции.
